...
class WebSocket {
	...
	public function __construct($address = null, $port = 8080) {
	...
	public function getClientsByPath($path = "/") {
		$clients = array_filter($this->getClients(), function($client) use($path) {
			if ($client->path == $path) {
				return true;
			}
			return false;
	 ...
	 */
	function setEnableLogging($enableLogging) {
	 ...
	 * @return bool
	 */
	public function setOption($socket, $level, $optname, $optval) {
	...
	public function connect($path = "/", $origin = null) {
		...
		$client->headers = $this->parseHeaders($buf);

		if (isset($client->headers['Sec-WebSocket-Accept'])) {
			$secWebSocketAccept = $client->headers['Sec-WebSocket-Accept'];
			...
			if (in_array($this->socket, $read) && $this->type == "server") {
				...
				if (!$client->handshake) {
					if ($this->processClientHandshake($client, $buf)) {
						if ($this->checkOrigin($client->headers)) {
							$this->clients[] = $client;
...
//			sleep(1);
	...
	public function sendData($client, $data) {
		if ($this->checkClientExistBySocket($client->socket)) {
			$response = $this->hybi10Encode($client, $data, "text");
			$this->write($client->socket, $response);
			$this->trigger("send", [
				$client,
				$data
			]);
	 ...
	 * @param object $client
	 */
	private function log($message, $client = null) {
	 ...
	 * @return boolean
	 */
	private function checkClientExistBySocket($socket) {
		foreach ($this->clients as $client) {
			if ($client->socket == $socket) {
				return true;
			}
	...
	private function checkOrigin($headers) {
	...
	private function sendHttpResponse($client, $httpStatusCode = 400) {
		$httpHeader = 'HTTP/1.1 ';
		...
		$httpHeader .= "\r\n";
		$this->write($client->socket, $httpHeader);
	}

	private function processClientHandshake($client, $input) {
		$matches = [];
		preg_match("/GET (.*) HTTP/i", $input, $matches);
		if (isset($matches[1])) {
			$client->path = trim($matches[1]);
		}
		$client->headers = $this->parseHeaders($input);

		if (!isset($client->headers['Sec-WebSocket-Key'])) {
			return false;
		}
		...
		if (!$this->write($client->socket, $upgrade)) {
...
//		socket_clear_error($socket);
	...
	private function hybi10Encode($client, $payload, $type = 'text', $masked = false) {
		$frameHead = array();
		$frame = '';
	...
	private function hybi10Decode($client, $data) {
		$payloadLength = '';
		$mask = '';
		$unmaskedPayload = '';
		$decodedData = array();
